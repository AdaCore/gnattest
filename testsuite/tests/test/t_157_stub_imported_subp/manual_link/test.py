...
from suite.tutils import build_harness, run_command, run_gnattest

BODY_STUB = Path("stubs", "simple", "pack.adb")
SPEC_STUB = Path("stubs", "simple", "pack.ads")

run_command("gprbuild", gpr="dep.gpr", args=["-q"])
...
thistest.fail_if(
    BODY_STUB.exists(),
    "When --no-external-stub is passed, spec with only imported subprograms "
    + "should not be eligible for stubbing",
)
thistest.fail_if(
    SPEC_STUB.exists(),
