...
def check(actual, expected):
    if actual != expected:
        print(
            "KO: Expected " + str(expected) + ", got " + str(actual) + " exit status."
        )
    else:
        print("OK")
...
def run_test(expected_exit_status, exit_val, passed_tests_val, msg):
    ...
    add_args = ["--exit-status=" + exit_val, "--passed-tests=" + passed_tests_val]

    run_args = ["gnattest", "obj/gnattest_stub/harness/test_drivers.list"] + add_args

    run_gnattest("test.gpr", ["--stub", "--tests-dir", "../tests", "-q"] + add_args)
