...
-- DATA STRUCTURES :
  ...
  type Natural_Number is new Float;
  type Natural32 is new Float;
  type Vec is array (integer32 range <>) of Integer;
  type VecVec is array (integer32 range <>) of Integer;
  type Link_to_VecVec is access Vec;
  ...
  type Node ( k : integer32 ) is record
    ...
    rows : Vec(1..k);                -- row positions on board
    cols : Vec(1..k);                -- column positions
