...
package body TGen.JSON is
         ...
         when J_NUMBER | J_INTEGER =>
            ...
            begin
               if Kind = J_INTEGER then
                  declare
                     Result_Int : Big_Integer;
                  begin
                     Result_Int := From_String (Number_Str);
   ...
   function Create (Val : Big_Integer) return JSON_Value is
      Ret : JSON_Value;
   begin
   ...
   function Create (Val : Big_Real) return JSON_Value is
      Ret : JSON_Value;
   begin
   ...
   function Get (Val : JSON_Value) return Big_Integer is
   begin
      return Val.Data.Int_Value;
   end Get;

   function Get (Val : JSON_Value) return Big_Real is
   begin
   ...
   function Get (Val : JSON_Value; Field : UTF8_String) return Big_Integer is
   begin
      return Get (Get (Val, Field));
   end Get;

   function Get (Val : JSON_Value; Field : UTF8_String) return Big_Real is
   begin
      return Get (Get (Val, Field));
   end Get;
   ...
   --  Copy of gnatcoll-json-utility.adb
