...
package TGen.Types is
   ...
   function Gen_Random_Function_Name (Self : Typ) return String
   is ("Gen_" & To_Ada (Self.Name));
   --  Return the name of the function generation random values for self in the
   --  sources for dynamic (two pass) generation

   function "<" (L : Typ'Class; R : Typ'Class) return Boolean
   is (To_Ada (L.Name) < To_Ada (R.Name));
   ...
   function Is_Constrained (Self : Typ) return Boolean
   is (False);
   ...
   function Default_Strategy
     (Self : Typ) return TGen.Strategies.Strategy_Type'Class;
   ...
   function Get_Diagnostics
     (Self : Typ; Prefix : String := "") return String_Vector
   is (String_Vectors.To_Vector
         (+To_Ada (Self.Name)
   ...
   function Supports_Gen (Self : Typ) return Boolean
   is (True);
   ...
   function JSON_Kind (Kind : Typ_Kind) return JSON_Value_Type
   is (case Kind is
   ...
   function Simple_Name (Self : Typ) return String
   is (+Unbounded_String (Self.Name.Last_Element));
   --  Return the simple name for the type

   type Scalar_Typ (Is_Static : Boolean) is new Typ with null record;

   function Get_Diagnostics
     (Self : Scalar_Typ; Prefix : String := "") return String_Vector
   ...
   function Supports_Gen (Self : Unsupported_Typ) return Boolean
   is (False);
   --  Unsupported types are unsupported for generation.

   function Kind (Self : Unsupported_Typ) return Typ_Kind
   is (Unsupported);
