...
package TGen.Libgen is
   ...
   with Pre => not (Bin_Tests and then On_Target);
   ...
   --
   --  When On_Target is True, the harness is generated to be executed on the
   --  emulated target: the generated JSON is streamed to the standard output
   --  (captured over the serial port by gnatemu) instead of being written to
   --  files, since there is no filesystem on target. The target and runtime
   --  are supplied by the caller on the gprbuild command line, so they are not
   --  embedded in the generated harness project. On_Target is only supported
   --  for JSON generation (not Bin_Tests).
   --
   --  If `On_Target` is set to True, building the generation harness requires
   --  adding `<Harness_Dir>/tgen_rts` to the GPR_PROJECT_PATH (or pass it
   --  through -aP to gprbuild), as well as setting the external variable
   --  TGEN_RTS_PROFILE to full.
   ...
   --  Markers used by TGen when dumping generated tests in JSON format through
   --  standard output. Used mostly for on-target test input generation to
   --  distinguish between the different JSON files.
