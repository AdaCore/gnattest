...
package TGen.Strings is
   ...
   is (Trim (Trim (Text, Right), Null_Set, To_Set (',')));

   function Remove_Trailing_Comma_And_Spaces (Text : String) return String
   is (Trim (Trim (Text, Right), Null_Set, To_Set (',')));
   ...
   procedure Write_Line
     (Str : in out Unbounded_String; Add : String; Span : Natural);
   ...
   procedure Write (Str : in out Unbounded_String; Add : String);
   --  Append the String Add to Str

   procedure Indent_String (Str : in out Unbounded_String; Span : Natural);
   ...
   function "=" (L, R : Ada_Identifier) return Boolean
   ...
   function "<" (L, R : Ada_Qualified_Name) return Boolean
   is (Ada.Strings.Less_Case_Insensitive (To_Ada (L), To_Ada (R)));
   ...
   package Ada_Qualified_Name_Sets is new
   ...
   function Map_Operator_Name (Op_Name : String) return String
   with Pre => Is_Operator (Op_Name);
   ...
   function To_JSON_filename (FQN : Ada_Qualified_Name) return String
   is (To_Filename (FQN) & ".json");
